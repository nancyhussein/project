...
class AIOVG_Public_Categories {
	...
	public function run_shortcode_categories( $atts ) {	
		...
		if ( 'grid' == $template ) {
            ...
            $doaa_term = get_term_by( 'slug', DOAA_AUDIO_CATEGORY,  "aiovg_categories");
            $quran_term = get_term_by( 'slug', QURAN_AUDIO_CATEGORY,  "aiovg_categories");
            if ( ! empty ( $doaa_term ) && ! is_wp_error( $doaa_term ) ){
                $args['exclude'] =  $doaa_term->term_id;
            }

            if ( ! empty ( $quran_term ) && ! is_wp_error(  $quran_term ) ){

                if(!isset($args['exclude']) && ! empty ( $args['exclude'] ) ){
                    $args['exclude'] =  $args['exclude'] .=$quran_term->term_id;
                }
                $args['exclude'] =  $quran_term->term_id;
            }
