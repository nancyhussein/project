 ...
 */
?>
    <h4 style="color: orange; text-align: center;"><?= $attributes['title'] ?></h4>
...
//                    printf( esc_html__( '%d views', 'all-in-one-video-gallery' ), $views_count );
                printf( esc_html__( '%d عدد المشاهدات', 'all-in-one-video-gallery' ), $views_count );
                ?>
            </div>
        <?php endif; ?>
...
if ( $attributes['related'] ) {
	...
	$atts[] = 'title="' . esc_html__( 'الفيديوهات الأخرى:', 'all-in-one-video-gallery' ) . '"';
